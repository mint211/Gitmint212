...
createNodes() {
  ...
  HOME_USER="/home/${USERNAME}"
	...
	idmax=`podman ps -a --format '{{ .Names}}' | awk -F "-" -v user="$USERNAME" '$0 ~ user"-debian" {print $3}' | sort -r |head -1`
...
dropNodes(){
  USERNAME=$1
	...
	# Update host
	scp /etc/hosts_template /etc/hosts
}
...
createAnsible(){
  ...
  infosNodes $USERNAME | awk '{print $NF"-"$1}' | awk -F- '{print "    pode"$4":"}' >> $ANSIBLE_DIR/00_inventory.yml
#  for conteneur in $(podman ps -a | grep $USERNAME-debian | awk '{print $1}');do      
 #   podman inspect -f '    {{.NetworkSettings.IPAddress }}:' $conteneur >> $ANSIBLE_DIR/00_inventory.yml
  #done
  mkdir -p $ANSIBLE_DIR/host_vars
  mkdir -p $ANSIBLE_DIR/group_vars
  # added by frank
  chown -R $USERNAME:$USERNAME $ANSIBLE_DIR 
	echo ""
	echo "# Added by createansible shell" >> /etc/hosts
# insert by frank updaed hosts by ip
infosNodes $USERNAME | awk '{print $NF"-"$1}' | awk -F- '{print $1" pode"$4}' >> /etc/hosts
awk '/node6/ {print "    "$NF":"}' /etc/hosts >> $ANSIBLE_DIR/00_inventory.yml
